...
| Region of attraction, \|u\| ≤ 10 N | **0.48 rad = 27.5°** from rest | The realistic limit. |
| Region of attraction, unlimited force | **55°** | But it demands **129 N** and **2.6 m** of track — not physical. |
...
$$
\begin{bmatrix} M+m & -ml\cos\theta \\[2pt] -ml\cos\theta & I_S+ml^2 \end{bmatrix}
\begin{bmatrix} \ddot p \\[2pt] \ddot\theta \end{bmatrix}
=
\begin{bmatrix} u - b\,\dot p - ml\,\dot\theta^{\,2}\sin\theta \\[2pt] mgl\sin\theta \end{bmatrix}
$$

Two results fell out of the algebra rather than being put in by hand, which makes them free correctness checks: Steiner's theorem appears on its own as the factor $I_S + ml^2$ multiplying $\ddot\theta$ — the inertia about the pivot, the point the pole physically swings about — and the mass matrix comes out symmetric, as it must, because kinetic energy is a quadratic form in the velocities.
...
I first derived the state matrix A and the input matrix B by hand and then checked them against their numerical Jacobian counterpart, which I calculated independently using finite differences in 'LQR.py'. They agree to `max |ΔA| = 4·10⁻¹²` and `max |ΔB| = 2·10⁻¹⁶`. The finite-difference version never looks inside `f` — it only calls the function and watches what comes out — so agreement means the derivation and the simulator are both right, rather than wrong in the same way.

The open-loop eigenvalues are `{0, 0, +3.974, −3.974}`. The positive one is the problem in a single number. Deviations grow proportional to $e^{3.974t}$, so the tilt doubles every 0.174 s, and any controller has to be faster than that. The two zeros are the cart. Since nothing restores its position, it drifts.
...
But even at angles up to 27.5° (with no initial angular speed), the saturated LQR controller was able to stabilize the pole, which shows that even though it's a linearized approach, it works quite well for the real, noninear system. 
...
Initially I also made the mistake of continuously checking whether the current state was within the boundaries of the strip. This only needed checking once, since it was possible for the angular speed to leave the strip during the process of the LQR. Outside the strip, control fell back to energy pumping, which is nearly silent near the top. I fixed this by handing control over to the LQR permanently once the state entered the strip a single time. Before the fix, 4 of 8 nearby configurations failed to balance at all.
